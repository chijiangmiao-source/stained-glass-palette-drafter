...
```
src/
  core/            # 纯逻辑：色板解析、像素映射、人工校色合成、导出文本、质量核查、备料核验、PNG 解码、布局常量
  components/      # GridView（可缩放编号网格）、StatsTable（用量统计）、QualityCheck（映射质量核查）、InventoryCheck（备料核验）
  App.vue          # 页面组装与状态流转
tests/
  unit/            # Vitest：色板校验、映射规则、导出格式、质量核查、备料核验
  e2e/             # Playwright：完整流程、错误处理、边界尺寸、导出下载、质量核查、备料核验
